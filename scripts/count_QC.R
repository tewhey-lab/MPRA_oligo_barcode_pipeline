...
for(celltype in unique(cell_reps$celltype)){
	...
	agg_rep_bcs[[celltype]] <- ggplot(agg_rep_bc, aes(x=Freq)) + geom_histogram(bins=200) + geom_vline(xintercept=10, col="red") + xlim(0,mean_bound) + xlab("Barcodes per aggregated Oligo") + ggtitle(paste0("Aggregated Barcode Count\n",agg_gt10," Oligos with > 10 Barcodes")) + theme_light()
	...
	for(rep in reps){
		...
		counts_bound <- as.numeric(quantile(indv_agg$counts, seq(0,1,0.01))[91])
