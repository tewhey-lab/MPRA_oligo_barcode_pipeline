...
task Pull_Barcodes {
  ...
  Int seq_min
  command {
    perl /scripts/pull_barcodes.pl ${merged_fastq} ${read_number} ${id_out}.merged ${barcode_link} ${oligo_link} ${end_oligo_link} ${seq_min}
    }
  output {
...
task SAM2MPRA {
  ...
  String id_out
  command {
    perl /scripts/SAM2MPRA.pl -C ${sam_file} ${id_out}.merged.match.enh.mapped
    }
  output {
...
task make_attr_list {
  ...
  String id_out
  command {
    perl /scripts/make_project_list.pl ${reference_fasta} ${id_out}
    }
  output {
...
task make_attr_file {
  ...
  String id_out
  command {
    perl /scripts/${oligo_type} ${proj_list} ${id_out}
    }
  output {
...
task prep_counts {
  ...
  String sample_id
  command {
    python /scripts/make_counts.py ${sample_fastq} ${parsed} ${sample_id}
    }
  output {
...
task associate {
  ...
  String sample_id
  command {
    perl /scripts/associate_tags.pl ${matched} ${parsed} ${sample_id}.tag
    }
  output {
...
task make_count_table {
  ...
  String id_out
  command {
    perl /scripts/compile_bc.pl ${flags} ${list_inFile} ${id_out}.count ${replicate_id}
    }
  output {
