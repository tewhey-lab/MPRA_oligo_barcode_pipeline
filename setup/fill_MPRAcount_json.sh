...
ID=`awk '{print $2}' ${FLOC}/<path/to/replicate_index> | uniq | awk '{aggr=aggr",\""$1"\""} END {print aggr}' | sed 's/,//'`
REPS=`awk '{print $2}' ${FLOC}/<path/to/replicate_index> | uniq | awk '{aggr=aggr",\"'${FLOC}'/fastq/"$1".fastq.gz\""} END {print aggr}' | sed 's/,//'`
...
jq --arg PROJ ${PROJ} --arg FLOC ${FLOC} --arg OUT ${OUT} --arg ID ${ID} --arg REPS ${REPS} --arg PARS ${PARS} -M '. + {"MPRAcount.out_directory":'\"${FLOC}/\"',
    "MPRAcount.id_out":'\"$OUT\"', "MPRAcount.parsed":'\"$PARS\"',
