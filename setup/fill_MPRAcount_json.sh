...
WORK="<path/to/MPRA_oligo_barcode_pipelin/scripts"
...
jq --arg PROJ ${PROJ} --arg FLOC ${FLOC} --arg ACC ${ACC} --arg OUT ${OUT} --arg ID ${ID} --arg REPS ${REPS} --arg PARS ${PARS} --arg WORK ${WORK} -M '. + {"MPRAcount.out_directory":'\"${FLOC}/\"',
    "MPRAcount.id_out":'\"$OUT\"', "MPRAcount.parsed":'\"$PARS\"', "MPRAcount.acc_id":'\"$ACC\"', "MPRAcount.working_directory":'\"$WORK\"', 
