...
WORK="<path/to/MPRA_oligo_barcode_pipeline/scripts"
...
jq --arg PROJ ${PROJ} --arg FLOC ${FLOC} --arg OUT ${OUT} --arg READ1 ${READ1} --arg READ2 ${READ2} --arg FASTA ${FASTA} --arg WORK ${WORK} -M '. + {"MPRAmatch.read_a":'\"${READ1}\"',
    "MPRAmatch.read_b":'\"${READ2}\"', "MPRAmatch.reference_fasta":'\"${FASTA}\"', "MPRAmatch.working_directory":'\"${WORK}\"',
