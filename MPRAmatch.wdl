...
task Parse {
  ...
  command {
    ...
    awk '($5 == 0)' ${counted} | awk -v lbrace="{" -v rbrace="}" 'lbrace ct[$2]++;cov[$2]+=$4; rbrace END lbrace for(i in ct) print i"\t"ct[i]"\t"cov[i] rbrace' > ${id_out}.merged.match.enh.mapped.barcode.ct.plothist
    }
  output {
